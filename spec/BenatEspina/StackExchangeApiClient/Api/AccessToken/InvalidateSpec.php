 ...
 */
class InvalidateSpec extends ObjectBehavior
{
    function it_can_be_invoked(HttpClient $client, Serializer $serializer)
    {
        $this->beConstructedWith($client, $serializer);
        $this->shouldHaveType(Invalidate::class);

        $client->get('/access-tokens/access-token-1;access-token-2/invalidate', [])->shouldBeCalled()->willReturn([]);
